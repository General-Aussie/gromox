...
EXMIDL(appt_meetreq_overlap, (const char *dir, const char *username, uint64_t start_time, uint64_t end_time, IDLOUT uint32_t *out_status))
EXMIDL(message_meeting_reply, (const char *dir, const char *frm, uint64_t mid, const MESSAGE_CONTENT *pmsgctnt, IDLOUT ec_error_t *pb_result))
